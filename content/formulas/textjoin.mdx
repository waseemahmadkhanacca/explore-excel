...
mistakes:
  - level: error
    ...
    body: TEXTJOIN arrived in Excel 2019. Open the file in 2016 and every TEXTJOIN becomes #NAME?. If the file leaves your organization, chain with ampersands instead or check what the recipient runs.
  - level: warning
...
With TRUE as the second argument, a missing middle name produces no double space. That is the behavior the old ampersand approach could not manage without extra IF logic.
