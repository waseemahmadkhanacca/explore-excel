...
mistakes:
  ...
  - level: warning
    ...
    body: EOMONTH plus a number of days gives an end-of-month-plus-terms date, but many organizations pay on a fixed day rather than a rolling one. Check the actual terms before building the logic.
  - level: warning
    title: Passing text instead of a date
    body: A date written as the text 2026-01-15 may work or may give #VALUE!, depending on regional settings. Use DATE(2026,1,15) or a real date cell so the behavior is the same everywhere.
faq:
